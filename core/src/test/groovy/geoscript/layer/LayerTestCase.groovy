...
class LayerTestCase {
    ...
    @Test void addFeaturesToLayerWithDifferentGeomName() {
        ...
        Workspace w = Workspace.getWorkspace([url: DataUtilities.fileToURL(file.parentFile)])
        Layer layer2 = w.create(schema2)
        layer1.eachFeature{ f ->
            layer2.add(f)
        }
        assertEquals 3, layer2.count
    ...
    @Test void reprojectToWorkspace() {
        ...
        // Reproject to a Workspace
        File file = folder.newFile("reproject.shp")
        Workspace workspace = Workspace.getWorkspace([url: DataUtilities.fileToURL(file.parentFile)])
        Layer layer2 = layer1.reproject(new Projection("EPSG:2927"), workspace, "facilities_reprojected")
        assertEquals 1, layer2.count()
        assertEquals 1144727.44, layer2.features[0].geom.x, 0.01
        assertEquals 686301.31, layer2.features[0].geom.y, 0.01
    }
    ...
    @Test void reprojectToLayer() {
        ...
        // Reproject to a Workspace Layer
        File file = folder.newFile("reproject.shp")
        Workspace workspace = Workspace.getWorkspace([url: DataUtilities.fileToURL(file.parentFile)])
        Layer propertyLayer = workspace.create(layer1.schema.reproject("EPSG:2927","facilities_epsg_2927"))
    ...
    @Test void constructors() {
        ...
        Workspace workspace = Workspace.getWorkspace(["url": url, namespace: 'http://omar.ossim.org'])
    ...
    @Test void minmax() {
        Workspace workspace = Workspace.getWorkspace([url: getClass().getClassLoader().getResource("states.shp")])
        Layer shp = workspace.get("states")
    ...
    @Test void cursorSorting() {
        ...
        Workspace workspace = Workspace.getWorkspace([url: DataUtilities.fileToURL(dir)])
        Layer layer = workspace.create('facilities',[new Field("geom","Point", "EPSG:2927"), new Field("name","string"), new Field("price","float")])
    ...
    @Test void getWriter() {
        ...
        Workspace w = Workspace.getWorkspace([url: DataUtilities.fileToURL(folder.newFolder("points"))])

...
class OGRTest {
    ...
    @Test void writeReadGeoJson() {
        if (shouldRunTest()) {
            File file = new File(FileUtil.createDir(folder,"states_geojson"), "states.geojson")
    ...
    @Test void writeReadKml() {
        if (shouldRunTest()) {
            File file = new File(FileUtil.createDir(folder,"states_kml"), "states.kml")
    ...
    @Test void writeReadGeoRSS() {
        if(shouldRunTest()) {
            File file = new File(FileUtil.createDir(folder,"states_georss1"), "states.georss")
    ...
    @Test void writeSpatialiteWithNewName() {
        if (shouldRunTest()) {
            File file = new File(FileUtil.createDir(folder,"states_spat"), "states.sqlite")
            OGR ogr = new OGR("Sqlite", file.absolutePath)
            File shpFile = new File(getClass().getClassLoader().getResource("states.shp").toURI())

...
class DirectoryStyleRepositoryTest {
    ...
    void css() {
        ...
        Style style = styleRepository.getDefaultStyleForLayer("states")
        assertNotNull(style)
        assertTrue(style.sld.startsWith("<?xml version=\"1.0\" encoding=\"UTF-8\"?><sld:StyledLayerDescriptor"))
        ...
        assertEquals("states", styles[0].layerName)
        assertEquals("states", styles[0].styleName)
        assertTrue(styles[0].style.sld.startsWith("<?xml version=\"1.0\" encoding=\"UTF-8\"?><sld:StyledLayerDescriptor"))

...
class TileGeneratorTestCase {
    ...
    void generateMissing() {
        File file = folder.newFile("colors.gpkg")
        GeoPackage geopkg = new GeoPackage(file, "colors", Pyramid.createGlobalMercatorPyramid())
        TileGenerator generator = new TileGenerator()
        // Generate all tiles with solid blue color
        generator.generate(geopkg, new ColorRenderer(Color.BLUE), 0, 2)
        (0..2).each { int z ->
            geopkg.tiles(z).each { ImageTile t ->
        ...
        // Delete zoom level 0
        geopkg.delete(geopkg.tiles(0))
        // Generate tiles but only the missing tiles (zoom level 0)
        generator.generate(geopkg, new ColorRenderer(Color.RED), 0, 2, missingOnly: true)
        // Zoom level 0 tiles should now be present but RED
        geopkg.tiles(0).each { ImageTile t ->
            assertNotNull t.data
